        ...
        <div class="box-header">
            @if (isset($_GET['msg']) && $_GET['msg'] != '')
                <div class="alert alert-danger">
                    <strong>Error deleting Module {{ $err_module ?? '' }}</strong><br> <?php echo $_GET['msg']; ?>
                </div>
            @endif
        </div>
        <div class="box-body">
            <table id="dt_modules" class="table table-bordered">
                ...
                <tbody>
                    ...
                    @foreach ($modules as $module)
                        <tr>
                            <td>{{ $module->id }}</td>
                            <td><a href="{{ url(config('laraadmin.adminRoute') . '/la_modules/' . $module->id) }}">{{ $module->label }}</a></td>
                            <td>{{ $module->name_db }}</td>
                            <td>{{ LAModule::itemCount($module->name) }}</td>
                            <td>
                                <a module_label="{{ $module->label }}" module_icon="{{ $module->fa_icon }}" module_id="{{ $module->id }}"
                                    class="btn btn-primary btn-xs update_module" style="display:inline;padding:2px 5px 3px 5px;"><i
                                        class="fa fa-edit"></i></a>
                                <a href="{{ url(config('laraadmin.adminRoute') . '/la_modules/' . $module->id) }}#access" class="btn btn-warning btn-xs"
                                    style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-key"></i></a>
                                <a href="{{ url(config('laraadmin.adminRoute') . '/la_modules/' . $module->id) }}#sort" class="btn btn-success btn-xs"
                                    style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-sort"></i></a>
                                <a module_name="{{ $module->name }}" module_id="{{ $module->id }}" class="btn btn-danger btn-xs delete_module"
    ...
    <div class="modal fade" id="AddExistModal" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                {!! Form::open(['route' => config('laraadmin.adminRoute') . '.la_modules.store', 'id' => 'module-add-form']) !!}
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            foreach ($tables as $table) {
                                $modItems = LAModule::where('name_db', $table)->first();
                                if (!isset($modItems)) {
    ...
    <div class="modal" id="module_update">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="module-update-form" role="form" action="{{ url('module_edit_submit') }}" class="smart-form" novalidate="novalidate"
                    method="post">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Module Name :</label>
                                <input type="text" class="form-control module_label_edit" placeholder="Module Name" name="Module Name"
                                    value="" />
                            </div>
                            <div class="form-group">
                                <label for="icon">Icon</label>
                                <div class="input-group">
                                    <input type="text" class="form-control module_icon_edit" placeholder="FontAwesome Icon" name="icon"
                                        value="" data-rule-minlength="1" required>
        ...
        $(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        for ($i = 0; $i < files.length; $i++) {
                            filesList += "<li>" + files[$i]['name'] + "<span class='pull-right " + files[$i]['class'] + "'>" +
                                files[$i]['perms'] + "</span>" + "</li>";
                        }
                        filesList += "</ul>";
            ...
            $('.update_module').on("click", function() {
                ...
                $('.save_edit_module').on("click", function() {
                    ...
                    $.ajax({
                        url: "{{ url(config('laraadmin.adminRoute') . '/la_module_update') }}",
                        type: "POST",
                        data: {
                            'id': module_id,
                            'label': module_label,
