...
@endsection
@section('contentheader_description', app('translator')->get('la_permission.permission_listing'))
@section('section', app('translator')->get('la_permission.permissions'))
@section('sub_section', app('translator')->get('common.listing'))
@section('htmlheader_title', app('translator')->get('la_permission.permission_listing'))

@section('headerElems')
    @la_access('Permissions', 'create')
        <button class="btn btn-success btn-sm pull-right" data-toggle="modal" data-target="#AddModal">@lang('la_permission.permission_add')</button>
    @endla_access
@endsection
...
@section('main-content')
    ...
    <div class="box box-success">
        ...
        <div class="box-body">
            <table id="dt_permissions" class="table table-bordered">
                <thead>
                    <tr class="success">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">@lang('la_permission.permission_add')</h4>
                    </div>
                    {!! Form::open(['action' => 'App\Http\Controllers\LA\PermissionsController@store', 'id' => 'permission-add-form']) !!}
...
@push('scripts')
    <script>
        ...
        $(function() {
            ...
            @la_access("Permissions", "create")
                // Create New Permission REST Request
                submitBtn = $('#permission-add-form button[type=submit]');
                formObj = $("#permission-add-form");
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (data.status == "success") {
                                    show_success("Permission Create", data);
                                    $('#AddModal').modal('hide')
                                    if (isset(data.redirect)) {
                                        window.location.href = data.redirect;
                                    }
                                } else {
                                    show_failure("Permission Create", data);
                                }
                                submitBtn.html('Save');
            ...
            @la_access("Permissions", "edit")
                ...
                dt_permissions.on('draw', function() {
                    $('.update_field').editable({
                        ...
                        validate: function(value) {
                            ...
                            $.ajax({
                                url: "{{ url(config('laraadmin.adminRoute')) }}/permissions/" + id,
                                method: 'PUT',
                                contentType: 'json',
                                headers: {
                                    'X-CSRF-Token': '{{ csrf_token() }}'
